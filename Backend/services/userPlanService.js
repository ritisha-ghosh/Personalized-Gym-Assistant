...
const isArrayEqual = (arr1, arr2) => {
  ...
  // Ensure we are working with clean, plain primitive strings mapping
  const cleanA = a.map(item => String(item).trim().toLowerCase());
  const cleanB = b.map(item => String(item).trim().toLowerCase());
  return JSON.stringify([...cleanA].sort()) === JSON.stringify([...cleanB].sort());
};
...
const getMLWorkoutRecommendation = async (user) => {
  try {
    const cleanDisease = user.medicalConditions && user.medicalConditions.length > 0 
      ? user.medicalConditions.filter(d => d && d !== 'Regular').join(' ') 
      : 'none';

    const cleanInjury = user.injuries && user.injuries.length > 0 
      ? user.injuries.filter(i => i && i !== 'Regular').join(' ') 
      : 'none';
    ...
    const response = await axios.post(`${ML_API_URL}/recommend-plan`, {
      ...
      disease: cleanDisease.trim() || 'none',
      injury: cleanInjury.trim() || 'none'
    }, { timeout: 6000 });
...
const getOrCreateWorkoutPlan = async (user) => {
  ...
  // ✅ CACHE INVALIDATION SAFE FIX: convert mongoose document states to plain JS structures before checking equality
  const snapshot = plan ? toPlainObject(plan.profileSnapshot) : null;
  ...
  const shouldRegenerate =
    ...
    !snapshot ||
    snapshot.goal       !== user.goal       ||
    snapshot.experience !== user.experience ||
    !isArrayEqual(snapshot.injuries,          user.injuries)          ||
    !isArrayEqual(snapshot.medicalConditions, user.medicalConditions);
...
const getOrCreateDietPlan = async (user) => {
  ...
  // ✅ DIET SNAPS FIX: convert mongoose layer documents to clean Javascript arrays before validation comparison
  const snapshot = dietPlan ? toPlainObject(dietPlan.profileSnapshot) : null;
  ...
  const shouldRegenerateDiet =
    ...
    !snapshot ||
    snapshot.goal         !== normalizeGoal(user.goal)         ||
    snapshot.dietType     !== normalizeDietType(user.dietType) ||
    snapshot.noOnion      !== !!user.noOnion                   ||
    snapshot.noGarlic     !== !!user.noGarlic                  ||
    snapshot.glutenFree   !== !!user.glutenFree                ||
    snapshot.lactoseFree  !== !!user.lactoseFree               ||
    snapshot.nutAllergy   !== !!user.nutAllergy                ||
    snapshot.sugarFree    !== !!user.sugarFree                 ||
    !isArrayEqual(snapshot.medicalConditions, user.medicalConditions) ||
    !isArrayEqual(snapshot.injuries,          user.injuries);
